...
(function(G) {
    ...
    class FretboardManager {
        ...
        add(options) {
            ...
            (function(mgr, fid, d) {
                d.addEventListener('click', function(e) {
                    if (parseFloat(getComputedStyle(d).opacity) < 0.5) return;
                    e.stopPropagation(); mgr.remove(fid);
                });
            })(this, fb.id, del);
            ...
            (function(mgr, fid, b) {
                b.addEventListener('click', function(e) {
                    if (parseFloat(getComputedStyle(b).opacity) < 0.5) return;
                    e.stopPropagation(); mgr.moveUp(fid);
                });
            })(this, fb.id, upBtn);
            ...
            (function(mgr, fid, b) {
                b.addEventListener('click', function(e) {
                    if (parseFloat(getComputedStyle(b).opacity) < 0.5) return;
                    e.stopPropagation(); mgr.moveDown(fid);
                });
            })(this, fb.id, downBtn);
            ...
            (function(mgr, fid) {
                copyBtn.addEventListener('click', function(e) {
                    if (parseFloat(getComputedStyle(copyBtn).opacity) < 0.5) return;
                    e.stopPropagation();
            ...
            (function(mgr, fid) {
                addBtn.addEventListener('click', function(e) {
                    if (parseFloat(getComputedStyle(addBtn).opacity) < 0.5) return;
                    e.stopPropagation();
                    var idx = mgr.count() + 1;
            ...
            tuneBtn.className = 'fretboard-tune';
            tuneBtn.innerHTML = '𝄞 ▼';
            tuneBtn.title = '调音设置';
            (function(mgr, fid, wrapper, b) {
                ...
                b.addEventListener('click', function(e) {
                    ...
                    // 将调音面板移到该指板下方
                    if (tp) {
                        wrapper.parentNode.insertBefore(tp, wrapper.nextSibling);
                        tp.style.display = '';
                        tp.classList.add('open');
                        setIcon(true);
                    }
                    if (G && G.updateTuningUI) G.updateTuningUI();
                });
            })(this, fb.id, wrapper, tuneBtn);
            wrapper.appendChild(tuneBtn);
            // wrapper 点击 → 激活 + 显示按钮
            (function(mgr, fid, w) {
        ...
        _loadState(fb) {
            ...
            // 恢复标注：只有有保存数据时才覆盖，避免快速切换时空值冲掉已有内容
            if (fb.annotations && fb.annotations.length > 0) fb.controller.annotations = fb.annotations.slice();
            if (fb.annotations && fb.annotations.length === 0) fb.controller.annotations = [];
        ...
        importAll(data) {
            ...
            if (mt && mt.setBpm) {
                ...
                // 刷新细分显示
                var ubsd = G.updateBeatSubdivDisplay;
                if (ubsd) ubsd();
            }
