        ...
        if (G.fretboardMgr) {
            G.fretboardMgr.getAll().forEach(function(fb) {
                ...
                if (t && t.type === 'scale') {
                    ...
                    var suffix = t.isNoteNameMode ? (currentLang === 'zh' ? ' (音名)' : ' (Note Name)')
                        : t.isSolfegeMode ? (currentLang === 'zh' ? ' (唱名)' : ' (Solfege)')
                        : (currentLang === 'zh' ? ' (音级)' : ' (Interval)');
                    title = currentLang === 'zh'
                        ? (t.baseTonic + '调 ' + (lang.scales[t.scaleType] || '') + ' ' + roman + '级' + chordName + suffix)
                        : (t.baseTonic + ' ' + (lang.scales[t.scaleType] || '') + ' ' + roman + ' ' + chordName + suffix);
                    fb.boardTitle = title;
                }
