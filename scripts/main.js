...
(function(G){
    ...
    let annotationMode = 'note';  // 'note' | 'solfege' | 'interval'
        ...
        if (G.fretboardMgr) {
            G.fretboardMgr.getAll().forEach(function(fb) {
                ...
                if (t && t.type === 'scale') {
                    ...
                    var suffix2 = annotationMode === 'note' ? (currentLang === 'zh' ? ' (音名)' : ' (Note Name)')
                        : annotationMode === 'solfege' ? (currentLang === 'zh' ? ' (唱名)' : ' (Solfege)')
    ...
    function updateAutoTitle() {
        ...
        if(isAutoTitle && controller && controller.currentTemplate && controller.currentTemplate.type) {
            if(controller.currentTemplate.type === 'scale') {
                ...
                var suffix = annotationMode === 'note' ? (currentLang === 'zh' ? ' (音名)' : ' (Note Name)')
                    : annotationMode === 'solfege' ? (currentLang === 'zh' ? ' (唱名)' : ' (Solfege)')
                    : (currentLang === 'zh' ? ' (音级)' : ' (Interval)');
    ...
    function toggleTheme() {
        // 以 G.currentTheme 为准（importAll 恢复后可能与其不同步）
        currentTheme = G.currentTheme === 'light' ? 'dark' : 'light';
    ...
    class FretboardRenderer {
        ...
        draw(annotations, currentTemplate, boardTitle) {
            ...
            for(let a of annotations) {
                ...
                this.ctx.stroke();
                var fs = (annotationMode === 'interval' && currentLang === 'zh') ? 25 : 28;
    ...
    class FretboardController {
        ...
        refreshAnnotationStyle(ann) {
            ...
            if (annotationMode === 'note') {
                ann.text = curNote;
                ann.bgColor = isRoot ? "#cc3333" : (tmpl.type === 'chord' ? "#c8e6c9" : "#f0f0f0");
                ann.textColor = isRoot ? "#ffffff" : (tmpl.type === 'chord' ? "#1b5e20" : "#2c2c2c");
            } else if (annotationMode === 'solfege') {
                ...
                ann.bgColor = isRoot ? "#cc3333" : "#f9e0a0";
        ...
        generateChord(key, scaleType, chordFamily, degree) {
            ...
            let suffix = annotationMode === 'note' ? (currentLang === 'zh' ? " (音名)" : " (Note Name)")
                : annotationMode === 'solfege' ? (currentLang === 'zh' ? " (唱名)" : " (Solfege)")
                : (currentLang === 'zh' ? " (音级)" : " (Interval)");
    ...
    function handlePointerDown(e) {
        ...
        // 只响应激活指板（非激活指板的标注绘制通过 dot 切换后处理）
        if (G.fretboardMgr) {
    ...
    function bindEvents() {
        ...
        document.getElementById('genBtnGroup')?.addEventListener('click', function(e) {
            ...
            if (btn) setAnnotationMode(btn.dataset.mode);
        });
        ...
        DOM.fretboardAddBtn.addEventListener('click', function() {
            if (!G.fretboardMgr) return;
            var idx = G.fretboardMgr.count() + 1;
            G.fretboardMgr.add({ name: currentLang === 'zh' ? '指板 ' + idx : 'Fretboard ' + idx });
            G.fretboardMgr.activate(G.fretboardMgr.count() - 1);
            console.log('[Fretboard] added, count=' + G.fretboardMgr.count() + ', activeIdx=' + G.fretboardMgr.activeIdx);
        });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 恢复后同步 annotationMode（importAll 已更新 G.annotationMode）
        if (G.annotationMode && G.annotationMode !== annotationMode) {
            setAnnotationMode(G.annotationMode);
        }
        ...
        if (!G.autoSave || !G.autoSave.hasSaved()) {
            DOM.accentToggle.classList.add('active');
            metro.setBeatPerBar(4);
            metro.setBpm(90);
            metro.setRhythmDiv(1);
...
// Expose runtime state & classes for other modules
    ...
    G.annotationMode = annotationMode;
